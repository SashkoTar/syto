...
class HighPassFilterTest {
  ...
  def shouldCalculateTwoOrderFilter() = {
    val (b, a) = calculateCoefficients(() => BesselMockedPrototypeRoots.twoOrder, 10.0)
    assertEquals(a(0), 1.0, 0.001)
    assertEquals(a(1), 17.3205080757, 0.001)
    assertEquals(a(2), 100.0, 0.001)
  ...
  def shouldCalculateThreeOrderFilter() = {
    val (b, a) = calculateCoefficients(() => BesselMockedPrototypeRoots.threeOrder, 10.0)
    assertEquals(a(0), 1.0, 0.001)
