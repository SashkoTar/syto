...
class AnalogChebyshevFirstTypeBandPassFilterTest {

  @Test
  def shouldCalculateTwoOrderFilter() = {
    val (b, a) = AnalogChebyshevFirstTypeBandPassFilter(2, 10f, 10f, 30f).calculateCoefficients()
  ...
  def shouldCalculateFiveOrderFilter() = {
    val (b, a) = AnalogChebyshevFirstTypeBandPassFilter(5, 10f, 10f, 30f).calculateCoefficients()
    assertEquals(a(0), 1.0, 0.001)
