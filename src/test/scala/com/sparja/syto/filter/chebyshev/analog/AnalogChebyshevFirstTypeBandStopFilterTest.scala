...
class AnalogChebyshevFirstTypeBandStopFilterTest {

  @Test
  def shouldCalculateTwoOrderFilter() = {
    val (b, a) = AnalogChebyshevFirstTypeBandStopFilter(2, 10f, 10f, 30f).calculateCoefficients()
  ...
  def shouldCalculateFiveOrderFilter() = {
    val (b, a) = AnalogChebyshevFirstTypeBandStopFilter(5, 10f, 10f, 30f).calculateCoefficients()
    assertEquals(a(0), 1.0, 0.001)
