...
class AnalogChebyshevSecondTypeBandStopFilterTest {

  @Test
  def shouldCalculateTwoOrderFilter() = {
    val (b, a) = AnalogChebyshevSecondTypeBandStopFilter(2, 10f, 13f, 29f).calculateCoefficients()
  ...
  def shouldCalculateFiveOrderFilter() = {
    val (b, a) = AnalogChebyshevSecondTypeBandStopFilter(5, 10f, 13f, 29f).calculateCoefficients()
    assertEquals(a(0), 1.0, 0.001)
