...
class HighPassFilterTest {
  ...
  def shouldCalculateTwoOrderFilter() = {
    val (b, a) = calculateCoefficients(() => EllipticMockedPrototypeRoots.twoOrder, 10.0)
    assertEquals(a(0), 1.0, 0.001)
    assertEquals(a(1), 7.38985470674, 0.001)
    assertEquals(a(2), 162.98980615, 0.001)
  ...
  def shouldCalculateThreeOrderFilter() = {
    val (b, a) = calculateCoefficients(() => EllipticMockedPrototypeRoots.threeOrder, 10.0)
    assertEquals(a(0), 1.0, 0.001)
    assertEquals(a(1), 44.710406363, 0.001)
    assertEquals(a(2), 216.536892549, 0.001)
